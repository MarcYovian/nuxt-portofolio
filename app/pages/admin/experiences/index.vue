...
// State
const columnFilters = ref([{ id: 'company', value: '' }])
...
watch(error, (newError) => {
  if (newError) {
    toast.add({
      ...
      description: newError.message || 'Failed to fetch experiences',
      color: 'error',
...
const columns: TableColumn<any>[] = [
  ...
  },
  {
    ...
    header: 'Logo',
    cell: ({ row }) => row.original.company_logo
      ? h('img', { src: row.original.company_logo, class: 'h-10 w-10 rounded object-cover border border-zinc-800' })
  ...
  },
  {
    ...
    cell: ({ row }) => h('div', { class: 'flex flex-col' }, [
      h('span', { class: 'font-medium text-highlighted' }, row.original.company),
...
<template>
  <NuxtLayout name="admin">
    <UDashboardPanel id="experiences">
      <template #header>
        <UDashboardNavbar title="Experiences">
          ...
          <template #right>
            <ExperiencesAddModal @success="refresh" />
          </template>
        </UDashboardNavbar>
      </template>
      ...
      <template #body>
        <div class="flex flex-wrap items-center justify-between gap-1.5">
          <UInput :model-value="(table?.tableApi?.getColumn('company')?.getFilterValue() as string)" class="max-w-sm"
            icon="i-lucide-search" placeholder="Filter companies..." :disabled="status === 'pending'"
            @update:model-value="table?.tableApi?.getColumn('company')?.setFilterValue($event)" />
        ...
        <div v-if="isEmpty" class="flex flex-col items-center justify-center py-12 text-center">
          ...
          <h3 class="text-lg font-semibold text-white mb-2">No Experiences Found</h3>
          <p class="text-zinc-400 mb-6">Get started by adding your first work experience.</p>
          <ExperiencesAddModal @success="refresh" />
        </div>
    ...
    <!-- Edit Modal -->
    <ExperiencesEditModal v-model:open="isEditOpen" :item="selectedItem" @success="refresh" />

    <!-- Delete Modal -->
    <ExperiencesDeleteModal v-model:open="isDeleteOpen" :items="selectedItems" @success="refresh" />
  </NuxtLayout>
</template>
