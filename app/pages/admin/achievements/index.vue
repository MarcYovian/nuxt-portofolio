...
watch(error, (newError) => {
    if (newError) {
        toast.add({
            ...
            description: newError.message || 'Failed to fetch achievements',
            color: 'error',
...
const columns: TableColumn<any>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => row.original.image_url
            ? h('img', { src: row.original.image_url, class: 'h-10 w-10 rounded object-cover border border-zinc-800' })
    ...
    },
    {
        ...
        cell: ({ row }) => h('div', { class: 'flex flex-col' }, [
            ...
            h('span', { class: 'text-xs text-muted' }, row.original.issuer)
        ])
    },
    {
        accessorKey: 'date',
    ...
    },
    {
        accessorKey: 'type',
        header: 'Type',
        cell: ({ row }) => h(UBadge, { variant: 'subtle', color: 'neutral' }, () => row.original.type || '-')
    },
    {
        accessorKey: 'tags',
        header: 'Tags',
        cell: ({ row }) => {
            ...
            const badges = displayTags.map((tag: any) =>
                h(UBadge, {
                    variant: 'subtle',
                    color: 'neutral',
                    class: 'mr-1',
                    style: { borderColor: tag.color, color: tag.color } // Optional: use tag color
                }, () => tag.name)
            )

            if (remaining > 0) {
                badges.push(h(UBadge, { variant: 'subtle', color: 'neutral' }, () => `+${remaining}`))
            }
...
<template>
    <NuxtLayout name="admin">
        <UDashboardPanel id="achievements">
            <template #header>
                <UDashboardNavbar title="Achievements">
                    <template #leading>
                        <UDashboardSidebarCollapse />
                    </template>

                    <template #right>
                        <AchievementsAddModal @success="refresh" />
                    </template>
                </UDashboardNavbar>
            </template>
            ...
            <template #body>
                <div class="flex flex-wrap items-center justify-between gap-1.5">
                    <UInput :model-value="(table?.tableApi?.getColumn('title')?.getFilterValue() as string)"
                        class="max-w-sm" icon="i-lucide-search" placeholder="Filter achievements..."
                ...
                <div v-if="isEmpty" class="flex flex-col items-center justify-center py-12 text-center">
                    <UIcon name="i-lucide-award" class="size-16 text-zinc-700 mb-4" />
                    <h3 class="text-lg font-semibold text-white mb-2">No Achievements Found</h3>
                    <p class="text-zinc-400 mb-6">Get started by adding your first achievement.</p>
                    <AchievementsAddModal @success="refresh" />
                </div>
                ...
                <UTable v-else ref="table" v-model:column-filters="columnFilters"
                    ...
                    <template v-if="status === 'pending'" #[`cell-image_url`]>
        ...
        <!-- Edit Modal -->
        <AchievementsEditModal v-model:open="isEditOpen" :item="selectedItem" @success="refresh" />

        <!-- Delete Modal -->
        <AchievementsDeleteModal v-model:open="isDeleteOpen" :items="selectedItems" @success="refresh" />
    </NuxtLayout>
</template>
