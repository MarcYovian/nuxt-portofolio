...
async function onSubmit(event: FormSubmitEvent<Schema>) {
    ...
    try {
        ...
        toast.add({
            title: 'Error',
            description: error.data?.message || error.message || 'Failed to create education',
            color: 'error',
...
<template>
    <UModal v-model:open="open" title="New Education" description="Add a new education record"
        :ui="{ content: 'sm:max-w-3xl' }">
        <UButton label="New Education" icon="i-lucide-plus" />
            ...
            <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
                ...
                <div class="flex justify-end gap-2 pt-4 border-t border-zinc-800">
                    <UButton label="Cancel" color="neutral" variant="subtle" :disabled="loading"
                        @click="open = false" />
                    <UButton label="Create Education" color="primary" variant="solid" type="submit"
                        :loading="loading" />
                </div>
            </UForm>
        </template>
