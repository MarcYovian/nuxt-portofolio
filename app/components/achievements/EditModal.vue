...
watch(() => open.value, async (isOpen) => {
    if (isOpen && props.item) {
        ...
        state.issuer = props.item.issuer
        state.date = props.item.date
        state.description = props.item.description
        state.credential_id = props.item.credential_id
        state.credential_url = props.item.credential_url
        state.type = props.item.type
        state.is_active = props.item.is_active
        state.is_featured = props.item.is_featured
        state.display_order = props.item.display_order
        state.image = undefined // Reset image input

        // Map existing tags
        state.tags = props.item.tags?.map((t: any) => t.id) || []
    }
})
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
    ...
    try {
        ...
        await $fetch(`/api/achievements/${props.item.id}`, {
            method: 'PUT',
            body: formData
        })
        ...
        toast.add({
            title: 'Error',
            description: error.data?.message || error.message || 'Failed to update achievement',
            color: 'error',
...
<template>
    <UModal v-model:open="open" title="Edit achievement" description="Update achievement details"
            ...
            <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <UFormField label="Type" name="type">
                        <USelectMenu v-model="state.type"
                            :options="['certification', 'award', 'publication', 'patent', 'course', 'other']"
                            class="w-full" />
                    </UFormField>
                    ...
                    <UFormField label="Image" name="image" class="col-span-full"
                        ...
                        <div v-if="item?.image_url" class="mb-2">
                            <img :src="item.image_url" class="h-20 w-auto rounded border border-zinc-800" />
                        </div>
