...
watch(() => props.item, (newItem) => {
    if (newItem) {
        ...
        state.price_range = newItem.price_range
        state.features = Array.isArray(newItem.features) ? [...newItem.features] : []
        state.icon = newItem.icon
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
    ...
    loading.value = true
    try {
        const response = await $fetch(`/api/services/${props.item.id}`, {
            method: 'PUT',
            body: event.data
        })
        ...
        toast.add({
            title: 'Error',
            description: error.data?.message || error.message || 'Failed to update service',
            color: 'error',
...
<template>
    <UModal v-model:open="isOpen" title="Edit service" description="Edit existing service"
