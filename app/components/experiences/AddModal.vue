...
async function onSubmit(event: FormSubmitEvent<Schema>) {
    ...
    try {
        ...
        toast.add({
            title: 'Error',
            description: error.data?.message || error.message || 'Failed to create experience',
            color: 'error',
            icon: 'i-lucide-alert-circle'
        })
    } finally {
...
<template>
    <UModal v-model:open="open" title="New Experience" description="Add a new work experience"
        :ui="{ content: 'sm:max-w-3xl' }">
        <UButton label="New Experience" icon="i-lucide-plus" />
            ...
            <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <UFormField label="Company Logo" name="image" class="col-span-full">
                        <UFileUpload icon="i-lucide-image" label="Drop company logo here"
                ...
                <div class="flex justify-end gap-2 pt-4 border-t border-zinc-800">
                    <UButton label="Cancel" color="neutral" variant="subtle" :disabled="loading"
                        @click="open = false" />
                    <UButton label="Create Experience" color="primary" variant="solid" type="submit"
                        :loading="loading" />
                </div>
            </UForm>
        </template>
